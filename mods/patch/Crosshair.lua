...
Crosshair = {
	SETTINGS = {
		...
		HS_MARKER_COLOR = {
			["save"] = "cb_crosshair_hs_marker_color",
			["widget_type"] = "dropdown",
			["text"] = "Color",
			["tooltip"] =  "Headshot marker color\n" ..
				"Changes the color of the headshot markers.",
			["value_type"] = "number",
			["options"] = {
				{text = "Default", value = Default},
				{text = "Red", value = Red},
...
Crosshair.create_options = function()
	...
	Mods.option_menu:add_item("hud_group", mod.SETTINGS.COLOR)
	...
	Mods.option_menu:add_item("hud_group", mod.SETTINGS.HS_MARKER_COLOR)
end
...
local change_crosshair_color = function(crosshair_ui)
	...
	crosshair_ui.crosshair_right.style.color = colors[get(mod.SETTINGS.COLOR)]

	if not crosshair_ui.hit_marker_animations[1] then
		for i,v in ipairs(crosshair_ui.hit_markers) do
		  v.style.rotating_texture.color = table.clone(colors[get(mod.SETTINGS.COLOR)])
	...
	if Crosshair.headshot_widgets[1] and not Crosshair.headshot_animations[1] then
		for i = 1, 4 do
			Crosshair.headshot_widgets[i].style.rotating_texture.color = table.clone(colors[get(mod.SETTINGS.HS_MARKER_COLOR)])
...
Mods.hook.set(mod_name, "CrosshairUI.draw_default_style_crosshair", function(func, self, ...)
	if get(mod.SETTINGS.DOT_ONLY) then
		return CrosshairUI.draw_dot_style_crosshair(self, ...)
	end
	populate_defaults(self)
	change_crosshair_scale(self)
	change_crosshair_color(self)
